...
> ![在线](https://img.shields.io/website?url=https%3A%2F%2Famtopa.com%2Findex-boundary&label=%E5%9C%A8%E7%BA%BF&color=35c6f0)
...
- **转生**：第一层重构（重置成长）+ 第二层跃迁（改变增长公式）+ 第三层法则重写（改写公式系数）+ 第四层法则彼岸（新货币维度）+ 第五层超维回响（印记货币，独立乘区商店）
- **大数**：自研 BigNumber（尾数 × 10^指数），支持 1e308 以上的超大数值
- **账号**：登录 + 云存档 + 排行榜
- **离线**：数学估算式离线收益，不做暴力逐 tick 模拟
- **维度**：三层跃迁完成后收集法则碎片跨入「法则彼岸」（Boss 自动攻击 + 彼岸商店）；彼岸收集回响印记跨入第 5 维度「超维回响」（回响商店 4 项独立乘区）
- **等级**：攻速/暴击按关卡设上限（100 关最多 100 级），攻击/金币无上限；天赋点满后自动转化为永久「天赋残辉」
